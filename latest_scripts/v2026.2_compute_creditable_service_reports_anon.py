...
OUTPUT_DETAIL = "output_detail_anon.csv"
OUTPUT_SUMMARY = "output_summary_anon.csv"
REPORT_DIR = Path("reports_anon")

PERSON_ID_COL = "Electronic Intrchg Person Id"  # EDIPI
PEBD_COL = "Pay Entry Base Date"                # reported PEBD (system)

# Optional: set to a specific EDIPI as string/int to filter,
# or None to process all EDIPIs.
TARGET_EDIPI = "1505877221"  # e.g., "1007461875"
...
def adjust_day_30(day: int, month: int, year: int, is_start: bool) -> int:
    """
    30-day month normalization (DODFMR style):
    - End date 31 -> 30
    - Feb 28 (non-leap) and Feb 29 -> 30
    - Start date 31 stays 31 (per your requirement)
    """
    if month == 2:
        is_leap = (year % 4 == 0 and (year % 100 != 0 or year % 400 == 0))
        if (not is_leap and day == 28) or day == 29:
            return 30
    if not is_start and day == 31:
        return 30
    return day
...
def compute_segment_days_30(start: date, end: date) -> int:
    ...
    ds = adjust_day_30(ds, ms, ys, is_start=True)
    de = adjust_day_30(de, me, ye, is_start=False)
    return (ye - ys) * 360 + (me - ms) * 30 + (de - ds) + 1  # inclusive
...
def merge_creditable_segments(df_person: pd.DataFrame) -> pd.DataFrame:
    """
    Collapse overlapping / back-to-back creditable segments into
    non-overlapping spans for one EDIPI. Lost-time rows are left as-is.

    DEP rows are already non-creditable (COMP not in valid_comp),
    so they never enter the merge.
    """
    ...
    cred = cred.sort_values(
        by=["Service Data From Dt", "Service Data To Dt"],
        ascending=[True, True],
    )
    ...
    for idx, (s, e_eff) in enumerate(merged, start=1):
        ...
        row["segment_days_actual"] = (
            (e_eff - s).days + 1 if pd.notna(e_eff) and pd.notna(s) else 0
        )
        row["segment_days_30"] = compute_segment_days_30(s, e_eff)
        # lost_time_days stays 0 in merged spans; summary uses df_detail
...
def main():
    ...
    if TARGET_EDIPI is not None:
        df = df[df[PERSON_ID_COL].astype(str) == str(TARGET_EDIPI)].copy()
        if df.empty:
            print(f"No rows found for EDIPI {TARGET_EDIPI}")
            return

    # -----------------------------
    # Anonymize EDIPI for outputs
    # -----------------------------
    unique_ids = (
        df[PERSON_ID_COL]
        .astype(str)
    ...
    id_map = {eid: f"M{idx+1:05d}" for idx, eid in enumerate(unique_ids)}
    df["anon_id"] = df[PERSON_ID_COL].astype(str).map(id_map)
    ...
    as_of = date.today()  # as-of date, treated as inclusive for PRESENT rows

    def segment_30_with_present(r):
        start = r["Service Data From Dt"]
        end = r["Service Data To Dt"]
        if end is None or pd.isna(end):
            # treat PRESENT as inclusive: use as_of as the effective end
            end = as_of
    ...
    # OLD net creditable days per row (this is what you used before)
    ...
    # ------------------------------------------------------------------
    # SUMMARY FOR PEBD (OLD BASIS: sum of net_creditable_days)
    # ------------------------------------------------------------------
    summary_pebd = (
        df_detail.groupby(PERSON_ID_COL, as_index=False)
          .agg(
              total_creditable_days=("net_creditable_days", "sum"),
          )
    )

    # ------------------------------------------------------------------
    # DISPLAY SUMMARY (GROSS, LOST, DISPLAY NET)
    # ------------------------------------------------------------------
    summary_display = (
        df_detail.groupby(PERSON_ID_COL, as_index=False)
          .agg(
    ...
    # Attach anon_id for convenience
    summary["anon_id"] = summary[PERSON_ID_COL].astype(str).map(id_map)

    # Use PEBD net (total_creditable_days) for Y/M/D and PEBD calc
    ...
    # ------------------------------------------------------------------
    # COMPUTE "CALCULATED PEBD" FROM AN AS-OF DATE (TODAY), USING PEBD NET
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # WRITE CSV OUTPUTS (use anon_id prominently)
    # ------------------------------------------------------------------
    detail_cols = list(df_detail.columns)
    detail_cols = [c for c in detail_cols if c != PERSON_ID_COL]
    df_detail[detail_cols].to_csv(OUTPUT_DETAIL, index=False)

    summary_cols = list(summary.columns)
    summary_cols = [c for c in summary_cols if c != PERSON_ID_COL]
    summary[summary_cols].to_csv(OUTPUT_SUMMARY, index=False)

    # ------------------------------------------------------------------
    # GENERATE PER-PERSON TEXT REPORTS
    #   - If TARGET_EDIPI is None: limit to first 500 EDIPIs.
    #   - If TARGET_EDIPI is set: only that one will be present anyway.
    # ------------------------------------------------------------------
    REPORT_DIR.mkdir(exist_ok=True)
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    if TARGET_EDIPI is None:
        summary_for_reports = summary.head(500)
    else:
        summary_for_reports = summary  # already filtered

    for _, row in summary_for_reports.iterrows():
        edipi = str(row[PERSON_ID_COL])           # internal
        anon = str(row["anon_id"])               # external label
        ...
        sub = df_detail[df_detail[PERSON_ID_COL] == edipi].copy()
        ...
        lines.append("")
        lines.append(
            f"ID: {anon:<15} Reported PEBD: {format_date(reported_pebd):<15}  AFADBD: See MCTFS TBIR"
        )
        lines.append("=" * 80)
        lines.append("")
        ...
        for _, s in sub.iterrows():
            ...
            if pd.isna(s["Service Data To Dt"]):
                to_str = "PRESENT"
            lost_str = (
                f"{int(s['lost_time_days'])}d"
                if s["lost_time_days"] and int(s["lost_time_days"]) > 0
                else "N/A"
            )
            lines.append(
        ...
        report_path = REPORT_DIR / f"statement_of_service_{anon}.txt"
        report_path.write_text("\n".join(lines), encoding="utf-8")

    print("Done.")
    if TARGET_EDIPI is not None:
        print(f"Filtered EDIPI: {TARGET_EDIPI}")
